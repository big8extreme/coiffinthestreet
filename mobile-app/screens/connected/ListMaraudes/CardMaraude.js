...
export default class CardMaraude extends Component {
    constructor(props) {
        super(props);
        this.state = {
          maraudes: []
      ...
      componentDidMount() {
        Axios.get(`${baseUrlApi}/maraudes`)
          .then(response => {
            this.setState({ maraudes: response.data.maraudes });
          })
          .catch(error => {
            console.log(error);
          });
    ...
    render() {
        return (
          ...
          {this.state.maraudes.map((maraude, index) => {
            return (
              <View key={index} style={styles.shadow}>
                ...
                >
                  ...
                  ><Text style={{ color: '#929292' }}>Le :</Text>
                    <Text style={styles.screen}>{moment(maraude.startAt)
                      .format("DD/MM/YYYY")}</Text>
                      <Text style={{ color: '#929292', marginRight: 30, marginLeft: 30 }}>à</Text>
                      <Text>{moment(maraude.startAt)
                      .format("HH[h]mm")}</Text>
                  </Text>
                  <Text
                    style={{
                ...
                />
                <View>
                <Svg height={100} width={400} marginTop={10} style={{ position: 'relative' }}>
                  <Svg.Path
